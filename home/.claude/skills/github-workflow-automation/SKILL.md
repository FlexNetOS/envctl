...
<!-- FlexNetOS load provenance + policy binding (prepended on load — NOT from upstream) -->
> **Loaded into the FlexNetOS fleet-wide skill surface** (`~/.claude/skills`, ADR-0006 → `envctl/home/.claude/skills`).
> Source: claude-flow/ruv-swarm github catalog, copied out of the `meta-ruvector` worktree
> `mrv--teas-engine` (that repo is FROZEN — copy-out only, never push back). `npx` has been
> normalized to `bunx` per the toolchain doctrine (bun-first; `bun-rewrite.sh` also enforces this live).
>
> **BINDING CONTRACT — every github action this skill performs is governed by the FlexNetOS GitHub
> execution policy** (see the `github` skill + `~/.claude/rules/git-topology.md`): never cherry-pick;
> strict upgrade-only (no removal / no comment-out / no permission-widening to sidestep a gate);
> meta worktree ritual; route fleet git through `rtk meta git`; **org SSH for all GitHub management**;
> land every change as a PR to `develop` with auto-merge, tick-on-merged, then reap the branch+worktree;
> GitHub workflows are Linux-only (no macOS/Windows runners); forks sync from upstream as supersets
> (never force-reset). Where a swarm recipe below conflicts with this contract, the contract wins.
...
bunx ruv-swarm actions generate-workflow \
  --analyze-codebase \
  --detect-languages \
...
bunx ruv-swarm actions optimize \
  --workflow ".github/workflows/ci.yml" \
  --suggest-parallelization

# Analyze failed runs
gh run view <run-id> --json jobs,conclusion | \
  bunx ruv-swarm actions analyze-failure \
    --suggest-fixes
```
...
```bash
# Usage example
bunx claude-flow@alpha github gh-coordinator \
...
gh pr create --title "Feature: New capability" \
  ...
  bunx ruv-swarm actions pr-validate \
...
bunx claude-flow@alpha github issue-tracker \
...
bunx claude-flow@alpha github release-manager \
...
bunx claude-flow@alpha github repo-architect \
...
bunx claude-flow@alpha github ci-orchestrator \
...
```bash
# Security audit
bunx ruv-swarm actions security \
  --deep-scan \
  --compliance-check \
  --create-issues
```
  ...
  swarm-analysis:
    ...
    steps:
      ...
      - name: Analyze Changes
        run: |
          bunx ruv-swarm actions analyze \
            --commit ${{ github.sha }} \
            --suggest-tests \
            --optimize-pipeline
```
  ...
  detect-and-build:
    ...
    steps:
      ...
      - name: Detect Languages
        id: detect
        run: |
          bunx ruv-swarm actions detect-stack \
            --output json > stack.json

      - name: Dynamic Build Matrix
        run: |
          bunx ruv-swarm actions create-matrix \
            --from stack.json \
            --parallel-builds
```
  ...
  security-swarm:
    ...
    steps:
      - name: Security Analysis Swarm
        run: |
          SECURITY_ISSUES=$(bunx ruv-swarm actions security \
            --deep-scan \
            --format json)
  ...
  heal-pipeline:
    ...
    steps:
      - name: Diagnose and Fix
        run: |
          bunx ruv-swarm actions self-heal \
  ...
  progressive-deploy:
    ...
    steps:
      - name: Analyze Risk
        id: risk
        run: |
          bunx ruv-swarm actions deploy-risk \
            --changes ${{ github.sha }} \
            --history 30d

      - name: Choose Strategy
        run: |
          bunx ruv-swarm actions deploy-strategy \
  ...
  perf-swarm:
    ...
    steps:
      - name: Performance Analysis
        run: |
          bunx ruv-swarm actions perf-test \
            --baseline main \
            --threshold 10% \
    ...
    steps:
      - name: Multi-Agent Validation
        run: |
          ...
          RESULTS=$(bunx ruv-swarm actions pr-validate \
            --spawn-agents "linter,tester,security,docs" \
            --parallel \
            --pr-data "$PR_DATA")
...
jobs:
  release:
    ...
    steps:
      - name: Release Swarm
        run: |
          bunx ruv-swarm actions release \
            --analyze-changes \
            --generate-notes \
            --create-artifacts \
            --publish-smart
...
bunx ruv-swarm actions analytics \
  --workflow "ci.yml" \
  --period 30d \
...
bunx ruv-swarm actions cost-optimize \
  --analyze-usage \
  --suggest-caching \
...
bunx ruv-swarm actions failure-patterns \
  --period 90d \
  --classify-failures \
  --suggest-preventions
```
...
bunx ruv-swarm actions resources \
  --analyze-usage \
  --suggest-runners \
  --cost-optimize
```
...
- name: Swarm Test Selection
  run: |
    bunx ruv-swarm actions smart-test \
...
jobs:
  generate-matrix:
    ...
    steps:
      - id: set-matrix
        run: |
          MATRIX=$(bunx ruv-swarm actions test-matrix \
            --detect-frameworks \
            --optimize-coverage)
...
bunx ruv-swarm actions parallel-strategy \
...
bunx ruv-swarm actions predict \
  --analyze-history \
  --identify-risks \
  --suggest-preventive
```
...
bunx ruv-swarm actions recommend \
  --analyze-repo \
  --suggest-workflows \
...
bunx ruv-swarm actions auto-optimize \
  --monitor-performance \
  --apply-improvements \
  --track-savings
```
...
jobs:
  swarm-task:
    ...
    steps:
      - name: Initialize Swarm
        run: |
          bunx ruv-swarm init --topology ${{ inputs.topology }}
```
...
- name: Setup Swarm
  ...
  run: |
    bunx ruv-swarm init --config "$SWARM_CONFIG"
```
...
- name: Quick Fail Check
  run: |
    if ! bunx ruv-swarm actions pre-check; then
...
bunx ruv-swarm actions profile \
  --workflow "ci.yml" \
  --identify-slow-steps \
...
bunx ruv-swarm actions analyze-logs \
  --directory ./logs \
  --identify-errors
```
...
jobs:
  initialize:
    ...
    steps:
      - id: init
        run: |
          SWARM_ID=$(bunx ruv-swarm init --topology mesh --output json | jq -r '.id')
  ...
  backend:
    ...
    steps:
      ...
      - name: Backend Tests
        run: |
          bunx ruv-swarm agents spawn --type tester \
  ...
  security:
    ...
    steps:
      ...
      - name: Security Scan
        run: |
          bunx ruv-swarm agents spawn --type security \
            --task "Security audit" \
            --swarm-id ${{ needs.initialize.outputs.swarm-id }}
...
jobs:
  detect-changes:
    ...
    steps:
      ...
      - id: detect
        run: |
          PACKAGES=$(bunx ruv-swarm actions detect-changes \
            --monorepo \
            --output json)
...
bunx claude-flow@alpha github sync-coordinator \
...
#### Workflow Generation
```bash
bunx ruv-swarm actions generate-workflow [options]
...
#### Optimization
```bash
bunx ruv-swarm actions optimize [options]
...
#### Analysis
```bash
bunx ruv-swarm actions analyze [options]
...
#### Testing
```bash
bunx ruv-swarm actions smart-test [options]
...
#### Security
```bash
bunx ruv-swarm actions security [options]
...
#### Deployment
```bash
bunx ruv-swarm actions deploy [options]
...
#### Monitoring
```bash
bunx ruv-swarm actions analytics [options]

...
6. **CONTAINMENT BEFORE CAPABILITY.** No nested Claude sessions (hook-denied). Subagents and teammates never spawn agents (depth-1). Max 6 active agents. Budget ceiling: 80% of any rate-limit window. Kill switch: `/home/flexnetos/meta/src/envctl/home/bin/harness-halt.sh` (not on `PATH` — invoke by full path).

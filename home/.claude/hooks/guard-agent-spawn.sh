...
if [ "$ACTIVE" -ge "$MAX_ACTIVE_AGENTS" ]; then
  ...
  deny "Runaway containment: $ACTIVE agents already active machine-wide (cap $MAX_ACTIVE_AGENTS). Wait for completions or run the kill switch (/home/flexnetos/meta/src/envctl/home/bin/harness-halt.sh)."
fi

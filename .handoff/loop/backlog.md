...
- [ ] **TASK-0078 (P0) — real-home dotfile relocation inventory + safe bridge automation (PR #290/#291/#293/#296/#298/#299/#300/#301/#303/#304/#306/#307/#309/#310/#311/#312/#313/#314/#316/#317/#318/#319/#320/#321/#322 MERGED; PR #323 .n8n-claude-bridge MERGED; PR #325 .pki NSSDB automation; live migration deferred on open Chrome handles; PR #326 .lane sensitive classification MERGED; PR #328 .fxapp-gh-profile sensitive browser-profile classification MERGED; PR #327 .forge automation/live migration; PR #329 open-handle guard for .pki retry):** Current loop target for the owner goal "walk every dot file/folder under `/home/drdave` and relocate/bridge surgically into meta." PR #290 hardened the read-only audit into a machine-readable `--inventory` TSV that records every top-level real-home dot entry with `type/state/target_class/canonical_target/action/apply_safe`, while keeping mutation limited to proven-safe bridges (`~/.local`, `.local/bin`, canonical `.gitconfig`). PR #291 added `--inventory-summary` TSV so the loop can gate class-by-class migrations from stable counts. PR #293 added explicit opt-in `--apply-shell-dotfiles` behavior plus TDD coverage: only duplicate/safe shell dotfiles are canonicalized into `$META_ROOT` and bridged; differing canonical files remain owner-supervised. Live apply evidence 2026-06-27: relocated/bridged `.bash_logout`, `.profile`, `.zshenv`, `.zshrc` into `/home/drdave/Desktop/meta`; `changed=4`; PR #293 is MERGED (`04b9e8f`).
  ...
  This follow-up slice adds `--owner-supervised-managed-config-child-deep-status PATH`, a read-only deep identity TSV for managed `.config` child conflicts where both real-home state and `$ENVCTL_HOME_SOURCE/.config/<name>` exist. It closes the PR #361 review gap where the shallow one-level conflict summary can report `digest_match=yes` while deep contents still differ: fixture coverage proves an identical `same-app` directory routes to `deep_identical=yes` / `review-then-bridge-identical-managed-config-child`, while a differing `diff-app` directory remains `deep_identical=no` / `owner-review-real-home-config-child-merge-or-remove-before-bridge`, with `apply_command` intentionally empty. Verification PASS: `bash -n scripts/audit-meta-local-paths.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/p7.sh`, and `bash ci/gates/harness-scripts.sh`. Live non-mutating evidence on 2026-06-27 emitted 5 deep-status rows (`ghostty`, `kasetto`, `nushell`, `systemd`, `yazelix`), all currently `deep_identical=no`; `systemd` is now explicitly routed to owner review despite the shallow summary's `digest_match=yes`. No live `--bridge-*` apply was performed. PR #367.

  This cache-child safety slice gates `--migrate-cache-child NAME` on the component-manifest contract surfaced by PR #362/#363: a reviewed `manifest/components.d/cache-<component>.toml` must exist before a named cache-child migration can proceed. Missing manifests now dry-run as a refusal and fail closed on `--apply` while preserving the source and target; manifest-backed fixture coverage still proves the approved dry-run/apply path, fake-lsof open-handle refusal, and existing-target collision refusal. Verification PASS: red test first failed on the missing-manifest apply gap, then `bash -n scripts/audit-meta-local-paths.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/p7.sh`, and `bash ci/gates/harness-scripts.sh` all passed. Live non-mutating evidence on 2026-06-27: cache-child manifest status still emits 84 rows with `manifest_exists=no`, and `--migrate-cache-child .wasm-pack` now dry-runs `would refuse automatic cache-child migration because component manifest manifest/components.d/cache-wasm-pack.toml is missing`; no live cache-child apply was performed. PR #368.

  This follow-up cache-child safety slice validates the reviewed manifest content, not just the file's presence: `--migrate-cache-child NAME` now requires the hinted `manifest/components.d/cache-<component>.toml` to declare a matching `[[component]] id = "cache-<component>"`. Existing but unrelated/empty manifests dry-run as a refusal and fail closed on `--apply` while preserving source and target; fixture coverage locks the wrong-id refusal before open-handle/target-collision checks and updates approved fixtures to a minimal matching component manifest. Verification PASS: red test first failed on the wrong-id apply gap, then `bash -n scripts/audit-meta-local-paths.sh scripts/tests/test-meta-local-path-audit.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/harness-scripts.sh`, and `bash ci/gates/p7.sh` all passed. Live non-mutating evidence on 2026-06-28: `--migrate-cache-child .wasm-pack` still dry-runs a missing-manifest refusal, the cache manifest-status report emits 84 rows with `manifest_exists=no`, and no live cache-child apply was performed. PR #369.

  This follow-up read-only cache-child planning slice adds `--owner-supervised-cache-child-component-manifest-validation PATH`, a 14-field TSV that reports the expected component id (`cache-<component>`) alongside manifest existence and whether an existing manifest declares that id. Missing manifests route to `create-cache-component-manifest-before-migration`, matching manifests route to `review-existing-cache-component-manifest-before-migration`, and existing wrong/empty manifests route to `fix-cache-component-manifest-id-before-migration`; `apply_command` remains empty and no live cache-child apply is performed. TDD coverage starts red for the unknown flag, then locks missing/valid/wrong manifest fixture branches, validation-only mode, direct-child exclusions, and the 14-field schema. Verification PASS: `bash -n scripts/audit-meta-local-paths.sh scripts/tests/test-meta-local-path-audit.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/harness-scripts.sh`, and `bash ci/gates/p7.sh` all passed. Runtime PASS on 2026-06-28: the validation report emitted 85 lines (header + 84 cache-child rows), `manifest_exists_yes=0`, `manifest_declares_expected_id_yes=0`, `create_next_action=84`, `nonempty_apply=0`, blocker classes were unchanged (`open-handles=1`, `owner-supervised-cache=1`, `owner-supervised-managed-dotfile=1`, `owner-supervised-sensitive=7`), and no `--apply` was used. PR #370.

  This follow-up read-only cache-child planning slice adds `--owner-supervised-cache-child-component-manifest-scaffold PATH`, a 17-field TSV that extends manifest validation with deterministic escaped TOML `manifest_stub` content for missing cache-child component manifests. Missing manifests route to `owner-review-cache-component-manifest-scaffold` with `scaffold_kind=component-manifest-minimal` and `scaffold_status=stub-needs-owner-review`; existing matching manifests route to review with no stub; existing wrong/empty manifests route to fix-id with no stub. `apply_command` remains empty and no manifest or cache state is written. TDD coverage starts red for the unknown flag, then locks missing/valid/wrong manifest fixture branches, escaped stub content, scaffold-only mode, direct-child exclusions, and the 17-field schema. Verification PASS: `bash -n scripts/audit-meta-local-paths.sh scripts/tests/test-meta-local-path-audit.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/harness-scripts.sh`, and `bash ci/gates/p7.sh` all passed. Runtime PASS on 2026-06-28: live non-mutating audit returned `rc=0` with `meta-local audit: PASS warnings=10 changed=0 dot_entries=79`; the scaffold report emitted 85 lines (header + 84 cache-child rows), all 84 rows were missing/no-declared-id scaffold stubs with owner-review next action (`rows=84 missing=84 no_decl=84 scaffold=84 pending=84 stub=84 next=84 nonempty_apply=0`), excluded `.config`/nested/sensitive rows (`config=0 nested=0 sensitive=0`), and no `--apply` was used. PR #371.

  This follow-up managed-config review slice adds `--owner-supervised-managed-config-child-deep-diff-summary PATH`, a read-only 19-field TSV that converts managed `.config` child conflicts into aggregate deep-diff counts before any bridge is attempted. It reports deep entry/file totals, shared/real-only/managed-only entry counts, type-conflict counts, differing shared regular-file counts, `deep_identical`, owner supervision, and an intentionally empty `apply_command`, without emitting nested relative paths or file contents. TDD coverage starts red for the unknown flag, then locks the identical `same-app` branch (`deep_identical=yes`, `review-then-bridge-identical-managed-config-child`) and the differing `diff-app` branch (5/5 entries, 3/4 files, 4 shared entries, 1 real-only, 1 managed-only, 1 type-conflict, 2 differing files, `deep_identical=no`, `owner-review-real-home-config-child-deep-diff-before-bridge`). Verification PASS: `bash -n scripts/audit-meta-local-paths.sh scripts/tests/test-meta-local-path-audit.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/harness-scripts.sh`, and `bash ci/gates/p7.sh` all passed. Runtime PASS on 2026-06-28: live non-mutating audit emitted 5 rows (`ghostty`, `kasetto`, `nushell`, `systemd`, `yazelix`), all `deep_identical=no` and all routed to `owner-review-real-home-config-child-deep-diff-before-bridge`; aggregate evidence was `type_conflicts_sum=15`, `real_only_entries_sum=23`, `managed_only_entries_sum=0`, `differing_files_sum=0`, `bad_nf=0`, and `bad_apply=0`. No live `--bridge-*` apply was performed. PR #372.

  This follow-up cache-child materialization slice adds `--write-cache-child-component-manifest NAME`, an explicit owner-reviewed writer for one direct real-home `.cache/<name>` component manifest at a time. It writes only the deterministic minimal stub to `manifest/components.d/cache-<component>.toml`, remains dry-run by default, requires `--apply` to mutate, never moves cache state, and intentionally runs after `--migrate-cache-child` attempts so a single invocation cannot create the manifest and immediately migrate without review. TDD coverage starts red for the unknown flag, then locks dry-run no-op behavior, apply writing the expected TOML, idempotent existing matching manifests, existing wrong-manifest refusal, invalid path-like names, and missing-source refusal. Verification PASS: `bash -n scripts/audit-meta-local-paths.sh scripts/tests/test-meta-local-path-audit.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/harness-scripts.sh`, and `bash ci/gates/p7.sh` all passed. Runtime PASS on 2026-06-28: live non-mutating audit for `.wasm-pack` dry-ran `would write manifest/components.d/cache-wasm-pack.toml declaring cache-wasm-pack`, confirmed no manifest was created, validation still emitted 84 rows with `bad_apply=0`, and no live cache state was moved. PR #373.

  This follow-up reviewed-manifest slice adds the first committed cache-child component manifest, `manifest/components.d/cache-wasm-pack.toml`, declaring `cache-wasm-pack` for the live `.wasm-pack` cache child. TDD red: before the file existed, the live/non-mutating `--migrate-cache-child .wasm-pack` probe dry-ran a refusal because `manifest/components.d/cache-wasm-pack.toml` was missing, and the new fixture expectation failed. Green: after materializing only the reviewed manifest stub, the test suite proves `.wasm-pack` now clears the reviewed-manifest precondition in dry-run mode and reports the would-move/would-link path while preserving source and target. Verification PASS: `bash -n scripts/audit-meta-local-paths.sh scripts/tests/test-meta-local-path-audit.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/harness-scripts.sh`, and `bash ci/gates/p7.sh` all passed. Runtime PASS on 2026-06-28: live non-mutating audit emitted `DRY-RUN: would move /home/drdave/.cache/.wasm-pack to /home/drdave/Desktop/meta/.local/cache/.wasm-pack and link ...`, `changed=0`, validation row `.cache/.wasm-pack` had `manifest_exists=yes`, `manifest_declares_expected_id=yes`, `next_action=review-existing-cache-component-manifest-before-migration`, and an empty `apply_command`; `/home/drdave/.cache/.wasm-pack` still exists and `/home/drdave/Desktop/meta/.local/cache/.wasm-pack` is absent. No live `--migrate-cache-child --apply` was performed and no cache state was moved. PR #376.

  This follow-up reviewed-manifest slice adds `manifest/components.d/cache-starship.toml`, declaring `cache-starship` for the live `starship` cache child after candidate selection proved `zellij` was not safe for this slice because `$META_ROOT/.local/cache/zellij` already exists. TDD red: with the manifest temporarily absent, the live/non-mutating `--migrate-cache-child starship` probe dry-ran a refusal because `manifest/components.d/cache-starship.toml` was missing. Green: after materializing only the reviewed manifest stub, fixture coverage proves `starship` clears the reviewed-manifest precondition in dry-run mode and reports the would-move/would-link path while preserving source and target. Verification PASS: `bash -n scripts/audit-meta-local-paths.sh scripts/tests/test-meta-local-path-audit.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/loop-state.sh`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/harness-scripts.sh`, and `bash ci/gates/p7.sh` all passed. Runtime PASS on 2026-06-28: live non-mutating audit emitted `DRY-RUN: would move /home/drdave/.cache/starship to /home/drdave/Desktop/meta/.local/cache/starship and link ...`, `changed=0`, validation row `.cache/starship` had `manifest_exists=yes`, `manifest_declares_expected_id=yes`, `next_action=review-existing-cache-component-manifest-before-migration`, and an empty `apply_command`; `/home/drdave/.cache/starship` still exists and `/home/drdave/Desktop/meta/.local/cache/starship` is absent. No live `--migrate-cache-child --apply` was performed and no cache state was moved. PR #377 (merged 2026-06-28, `425c15fbb98752a70cfb5618654ecd3e43e17a29`).

  This follow-up reviewed-manifest slice adds `manifest/components.d/cache-agent-env.toml`, declaring `cache-agent-env` for the live `agent-env` cache child. TDD red: before the file existed, the live/non-mutating `--migrate-cache-child agent-env` probe dry-ran a refusal because `manifest/components.d/cache-agent-env.toml` was missing. Green: after materializing only the reviewed manifest stub, fixture coverage proves `agent-env` clears the reviewed-manifest precondition in dry-run mode and reports the would-move/would-link path while preserving source and target. Verification PASS: `bash -n scripts/audit-meta-local-paths.sh scripts/tests/test-meta-local-path-audit.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/loop-state.sh`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/harness-scripts.sh`, and `bash ci/gates/p7.sh` all passed. Runtime PASS on 2026-06-28: live non-mutating audit emitted `DRY-RUN: would move /home/drdave/.cache/agent-env to /home/drdave/Desktop/meta/.local/cache/agent-env and link ...`, `changed=0`, validation row `.cache/agent-env` had `manifest_exists=yes`, `manifest_declares_expected_id=yes`, `next_action=review-existing-cache-component-manifest-before-migration`, and an empty `apply_command`; `/home/drdave/.cache/agent-env` still exists and `/home/drdave/Desktop/meta/.local/cache/agent-env` is absent. No live `--migrate-cache-child --apply` was performed and no cache state was moved. PR #378 (merged 2026-06-28T03:23:00Z, `c716ff021412fff53b43f2e125c28982b3f9ff15`).

  This follow-up reviewed-manifest slice adds `manifest/components.d/cache-jna.toml`, declaring `cache-jna` for the live `JNA` cache child. TDD red: before the file existed, the live/non-mutating `--migrate-cache-child JNA` probe dry-ran a refusal because `manifest/components.d/cache-jna.toml` was missing. Green: after materializing only the reviewed manifest stub, fixture coverage proves `JNA` clears the reviewed-manifest precondition in dry-run mode and reports the would-move/would-link path while preserving source and target. Verification PASS: `bash -n scripts/audit-meta-local-paths.sh scripts/tests/test-meta-local-path-audit.sh`, `bash scripts/tests/test-meta-local-path-audit.sh`, `git diff --check`, `bash ci/gates/loop-state.sh`, `bash ci/gates/meta-local-policy.sh`, `bash ci/gates/harness-scripts.sh`, and `bash ci/gates/p7.sh` all passed. Runtime PASS on 2026-06-28: live non-mutating audit emitted `DRY-RUN: would move /home/drdave/.cache/JNA to /home/drdave/Desktop/meta/.local/cache/JNA and link ...`, `changed=0`, validation row `.cache/JNA` had `manifest_exists=yes`, `manifest_declares_expected_id=yes`, `next_action=review-existing-cache-component-manifest-before-migration`, and an empty `apply_command`; `/home/drdave/.cache/JNA` still exists and `/home/drdave/Desktop/meta/.local/cache/JNA` is absent. No live `--migrate-cache-child --apply` was performed and no cache state was moved. PR #379.

...
into clean), **C-13** init template writer (local + `--global`, `--force` overwrite) — implemented
as `Engine::agent_init` + `envctl agent init`.
...
- [x] C-13 · src/commands/init.rs:run + TEMPLATE + init_config_path · `init` (verb 9): write commented YAML template (local or `--global`); ERR on existing unless `--force`. Implemented as `Engine::agent_init` + `envctl agent init --global --force`. The TTY interactive overwrite prompt + banner are FRONT-END divergence; the engine fail-closes on existing files. · -> Engine::agent_init + CLI `agent init` · deps: XC-03,CP-01
...
- [x] FRONTEND-01 · src/cli.rs:Cli/Commands/SyncArgs/ScopeArgs/OutputArgs/ListKind/ColorMode/SelfAction · front-end: clap subcommand/flag wiring. CLOSED (TASK-0019): global `-q/--quiet`/`-v/--verbose`/`--color`/`--no-color` (`ColorMode` + `resolve_plain`), `self` subcommand tree (`SelfAction::{Update,Uninstall}`), `completions <shell>`, `--frozen` alias on the agent `--locked`/lock-`--check` flags, and `-f` short on `agent init` all landed in `crates/cli/src/main.rs`. VERB SEMANTICS captured by C-01..C-14. · -> envctl CLI · deps: C-01..C-14
- [x] FRONTEND-02 · src/app.rs:run + dispatch + should_suppress_notice + current_program_name · front-end: clap dispatch + update-notice gating. CLOSED (TASK-0019): `main()` spawns the update-notifier background check, `should_suppress_notice` gates the end-of-run notice (json/quiet/completions/self/env suppressed), `upgrade_command` ports the cargo/installer arms. · -> envctl CLI · deps: FRONTEND-01
...
- [x] FRONTEND-07 · src/update_notifier.rs · CLOSED (TASK-0019): ported to `crates/engine/src/update_notifier.rs` (non-printing cache/check core: spawn_background_check/wait_for_check/read_cached_entry/now_unix_secs/available_update; env override `KASETTO_CACHE_DIR`→`ENVCTL_CACHE_DIR`; 24h TTL; repo via the self-update core `FlexNetOS/envctl`) + CLI end-of-run notice render. Golden tests (cache round-trip, TTL boundary, available_update) ported. · -> engine update_notifier + CLI · deps: FRONTEND-08
- [x] FRONTEND-08 · src/commands/self_update.rs · CLOSED (TASK-0019): CORE ported to `crates/engine/src/self_update.rs` (fetch_latest_release/is_newer/current_target/verify_checksum/plan_self_update, GITHUB_REPO=`FlexNetOS/envctl`, asset names retargeted) reusing `envctl_agent_env::source::http_client` (zero new deps). CLI half (download/extract/atomic-replace, tar-slip guard, `.old` backup + restore-on-fail, 0o755) in `crates/cli/src/self_update.rs`. All 9 is_newer/verify_checksum/current_target golden vectors ported. · -> engine self_update + CLI · deps: none
- [x] FRONTEND-09 · src/commands/doctor.rs · CLOSED (TASK-0019): the kasetto `doctor` is the AGENT-ENV doctor (version/lock/scope/skills/install-path/last-sync/failures/mcps/commands/command-dir-writability/update-check) — ported as `Engine::agent_doctor` (`crates/engine/src/agent/doctor.rs`, `AgentDoctorReport`, `Event::AgentDoctored`, `AgentCommandSpec::Doctor`) + CLI `agent doctor [--scope]` grouped render + GUI Doctor sub-tab (parity). Distinct from envctl's existing component `doctor` (AP-03, untouched). is_writable/format_age boundary + DoctorOutput field-set vectors ported. · -> engine agent_doctor + CLI + GUI · deps: M-19, FRONTEND-07
- [x] FRONTEND-10 · src/commands/completions.rs · CLOSED (TASK-0019): `envctl completions <shell>` generates envctl's OWN clap tree via `clap_complete::generate` (pure-Rust dep `clap_complete = "4.5"`; no-c gate PASS). Per-shell non-empty + bin-name tests added. · -> envctl CLI · deps: FRONTEND-01

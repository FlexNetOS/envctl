...
- [≠] FRONTEND-01 · src/cli.rs:Cli/Commands/SyncArgs/ScopeArgs/OutputArgs/ListKind/ColorMode/SelfAction · front-end: clap subcommand/flag wiring. VERB SEMANTICS captured by C-01..C-14; envctl re-expresses as `envctl agent {sync,add,remove,lock,list,clean,init}` clap. `--dry-run`/`--json`/`--locked`/`--frozen`(alias)/`--update`/`--upgrade-package`/`--scope`/`--global`/`--force` flags are the no-downgrade flag surface — must reach the Engine methods. · -> envctl CLI (TASK-0014) · deps: C-01..C-14

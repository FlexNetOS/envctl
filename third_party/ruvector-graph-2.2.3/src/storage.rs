...
impl GraphStorage {
    ...
    pub fn insert_node(&self, node: &Node) -> Result<NodeId> {
        let write_txn = self.db.begin_write()?;
        {
            ...
            // Serialize node data
            let node_data = bincode_reloaded::encode_to_vec(node, config::standard())?;
    ...
    pub fn get_node(&self, id: &str) -> Result<Option<Node>> {
        ...
        let (node, _): (Node, usize) =
            bincode_reloaded::decode_from_slice(node_data.value(), config::standard())?;
        Ok(Some(node))
    }
    ...
    pub fn insert_edge(&self, edge: &Edge) -> Result<EdgeId> {
        let write_txn = self.db.begin_write()?;
        {
            ...
            // Serialize edge data
            let edge_data = bincode_reloaded::encode_to_vec(edge, config::standard())?;
    ...
    pub fn get_edge(&self, id: &str) -> Result<Option<Edge>> {
        ...
        let (edge, _): (Edge, usize) =
            bincode_reloaded::decode_from_slice(edge_data.value(), config::standard())?;
        Ok(Some(edge))
    }
    ...
    pub fn insert_hyperedge(&self, hyperedge: &Hyperedge) -> Result<HyperedgeId> {
        let write_txn = self.db.begin_write()?;
        {
            ...
            let hyperedge_data = bincode_reloaded::encode_to_vec(hyperedge, config::standard())?;
    ...
    pub fn get_hyperedge(&self, id: &str) -> Result<Option<Hyperedge>> {
        ...
        let (hyperedge, _): (Hyperedge, usize) =
            bincode_reloaded::decode_from_slice(hyperedge_data.value(), config::standard())?;
        Ok(Some(hyperedge))
    }

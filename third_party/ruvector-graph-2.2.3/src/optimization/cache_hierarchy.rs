...
impl ColdStorage {
    ...
    fn get(&self, node_id: u64) -> Option<NodeData> {
        self.entries.get(&node_id).and_then(|compressed| {
            // Decompress data using bincode_reloaded 2.0 API
            bincode_reloaded::decode_from_slice(&compressed, bincode_reloaded::config::standard())
                .ok()
                .map(|(data, _)| data)
        })
    }
    ...
    fn insert(&mut self, node_id: u64, data: NodeData) {
        // Compress data using bincode_reloaded 2.0 API
        if let Ok(compressed) = bincode_reloaded::encode_to_vec(&data, bincode_reloaded::config::standard()) {
...
/// Node data structure
#[derive(Clone, serde::Serialize, serde::Deserialize, bincode_reloaded::Encode, bincode_reloaded::Decode)]
pub struct NodeData {
    pub id: u64,

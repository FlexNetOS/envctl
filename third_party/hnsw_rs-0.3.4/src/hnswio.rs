...
// differ from v2 as we do not use fugue_bincode encoding for point. We dump pure binary
    ...
    T: 'static + DeserializeOwned + Clone + Sized + Send + Sync,
{
    ...
    let v: Vec<T> = if std::any::TypeId::of::<T>() != std::any::TypeId::of::<NoData>() {
        match descr.format_version {
            2 => fugue_bincode::deserialize(&v_serialized).unwrap(),
            3 | 4 => {
                let slice_t = unsafe {
...
mod tests {
    ...
    fn test_bincode() {
        ...
        let v_serialized: Vec<u8> = fugue_bincode::serialize(&data).unwrap();
        debug!("serializing len {:?}", v_serialized.len());
        let v_deserialized: Vec<f32> = fugue_bincode::deserialize(&v_serialized).unwrap();

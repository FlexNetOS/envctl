...
impl HnswIndex {
    ...
    /// Serialize the index to bytes using bincode_reloaded
    pub fn serialize(&self) -> Result<Vec<u8>> {
        ...
        bincode_reloaded::encode_to_vec(&state, bincode_reloaded::config::standard()).map_err(|e| {
            RuvectorError::SerializationError(format!("Failed to serialize HNSW index: {}", e))
        })
    }

    /// Deserialize the index from bytes using bincode_reloaded
    pub fn deserialize(bytes: &[u8]) -> Result<Self> {
        let (state, _): (HnswState, usize) =
            bincode_reloaded::decode_from_slice(bytes, bincode_reloaded::config::standard()).map_err(|e| {

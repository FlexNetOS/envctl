...
/// Note: Serialized using JSON (not bincode_reloaded) due to serde_json::Value in metadata field
...
#[derive(Debug, Clone, Serialize, Deserialize, bincode_reloaded::Encode, bincode_reloaded::Decode)]
pub struct Skill {
    pub id: String,
    pub name: String,
...
impl AgenticDB {
    ...
    pub fn store_episode(
        ...
        let write_txn = self.db.begin_write()?;
        {
            ...
            // Use JSON encoding for ReflexionEpisode (contains serde_json::Value which isn't bincode_reloaded-compatible)
            ...
            if let Some(metadata) = result.metadata {
                if let Some(episode_id) = metadata.get("episode_id") {
                    ...
                    if let Some(data) = table.get(id)? {
                        // Use JSON decoding for ReflexionEpisode (contains serde_json::Value which isn't bincode_reloaded-compatible)
    ...
    pub fn create_skill(
        ...
        let write_txn = self.db.begin_write()?;
        {
            ...
            let data = bincode_reloaded::encode_to_vec(&skill, bincode_reloaded::config::standard())
                .map_err(|e| RuvectorError::SerializationError(e.to_string()))?;
                ...
                if let Some(skill_id) = metadata.get("skill_id") {
                    ...
                    if let Some(data) = table.get(id)? {
                        let (skill, _): (Skill, usize) =
                            bincode_reloaded::decode_from_slice(data.value(), bincode_reloaded::config::standard())
                                .map_err(|e| RuvectorError::SerializationError(e.to_string()))?;
                        skills.push(skill);
                    }
                }
    ...
    pub fn add_causal_edge(
        ...
        let write_txn = self.db.begin_write()?;
        {
            ...
            let data = bincode_reloaded::encode_to_vec(&edge, bincode_reloaded::config::standard())
                .map_err(|e| RuvectorError::SerializationError(e.to_string()))?;
            table.insert(id.as_str(), data.as_slice())?;
        }
        write_txn.commit()?;
    ...
    pub fn start_session(
        ...
        let write_txn = self.db.begin_write()?;
        {
            ...
            let data = bincode_reloaded::encode_to_vec(&session, bincode_reloaded::config::standard())
                .map_err(|e| RuvectorError::SerializationError(e.to_string()))?;
            table.insert(id.as_str(), data.as_slice())?;
        }
        write_txn.commit()?;

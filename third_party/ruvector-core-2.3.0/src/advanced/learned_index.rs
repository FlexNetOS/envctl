...
impl HybridIndex {
    ...
    pub fn rebuild(&mut self) -> Result<()> {
        ...
        for (key_bytes, value) in &self.dynamic_buffer {
            let (key, _): (Vec<f32>, usize) =
                bincode_reloaded::decode_from_slice(key_bytes, bincode_reloaded::config::standard())
    ...
    fn serialize_key(key: &[f32]) -> Vec<u8> {
        bincode_reloaded::encode_to_vec(key, bincode_reloaded::config::standard()).unwrap_or_default()
    }
}

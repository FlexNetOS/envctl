...
configure_hermetic_build_environment() {
  ...
  cargo_rustflags="--remap-path-prefix=$build_workspace=/envctl-build --remap-path-prefix=$source_root=/envctl-source --remap-path-prefix=$M=/meta --remap-path-prefix=$private_cargo_home=/envctl-cargo-home -C codegen-units=1 -C embed-bitcode=yes -C llvm-args=-rng-seed=1 -C link-arg=--no-default-config -C link-arg=--ld-path=$hermetic_lld -C link-arg=-Wl,--build-id=sha1"
}

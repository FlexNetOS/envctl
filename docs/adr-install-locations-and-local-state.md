...
manifest declares the expected `[[component]] id = "cache-<component_key>"`. Missing manifests
must be created and reviewed before a named `--migrate-cache-child NAME` run; existing manifests
must be reviewed before use; existing manifests with a missing or unrelated component id must be
fixed before migration. These reports intentionally leave `apply_command` empty and do not move

...
> Renamed kasetto→`envctl agent`; `kasetto.yaml`→`agent-env.yaml`; mimalloc replaced with rust base equivalant.
...
| Scope | Location |
| --- | --- |
| Global | `$XDG_DATA_HOME/agent-env/agent-env.lock` (Envctl-managed default: `/home/flexnetos/meta/var/xdg-data/agent-env/agent-env.lock`) |

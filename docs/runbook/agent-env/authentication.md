...
> Source: https://www.kasetto.dev/docs/authentication. The standalone `kasetto` binary is retired — this is the `envctl agent` surface.

Auth tokens for private GitHub, GitLab, Bitbucket, Codeberg, and Gitea.

envctl can pull skills, commands, and MCP configs from private repositories. No login command or credentials file needed — just set an environment variable and it works. _(The upstream docs also mention `instructions`; that asset kind is upstream-only and not in the absorbed v3.2.0 surface.)_
...
envctl identifies the git host from the URL hostname:
...
```
export GITHUB_TOKEN=ghp_...
envctl agent sync --config https://github.com/org/private-repo/blob/main/agent-env.yaml --apply
```

If the URL points to a private resource and no matching token is set, envctl reports an HTTP error with a hint about which variable to set.

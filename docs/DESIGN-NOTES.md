...
- `add-repo` hardened: strict slug validation, refuse id-collision with a built-in, atomic temp+rename drop-in write with timestamped backup, clone into `$META_ROOT/var/lib/envctl/repos/<slug>` (0700) not `/tmp`, all interpolated values escaped.

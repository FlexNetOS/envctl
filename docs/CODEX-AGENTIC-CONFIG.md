...
| Hooks | Stop/PreCompact hooks used an absolute path into the main checkout. | Resolve hook script from `git rev-parse --show-toplevel`. | Worktrees now checkpoint their own repo instead of the stale main checkout. |
...
- Keep credentials, provider redirects, and private auth in `~/.codex/config.toml`; project config

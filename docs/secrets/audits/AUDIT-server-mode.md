...
(A19) are all correct in intent. Profile B (VPS) was correctly gated as non-shippable; **as of
TASK-0033 it is RESOLVED + shippable** — the operator-box authorizer protocol (OI-SM-2) and external
trusted time (OI-SM-3) are specified (`OI-SM-2-operator-authorizer.md`), built (`broker::authorizer`,
`secretd::edge::authorizer`, `secretctl authorizer`), and tested with passing FS-S21/S22/S23/S24
negatives (`crates/secretd/tests/profile_b_e2e.rs`).
...
| F7 | **RESOLVED (TASK-0033)** | VPS Profile B install-time gate landed (FS-S21) | `SERVER-MODE §5.3` lines 171-180; `ops/01 §8` (sketch only); `ops/02 §3` (`enable=false` until Phase 6); FS-S21 line 301 | Operator editing `secrets.toml` to `profile="remote"` without `operator_authorizer_url` silently downgrades to passphrase-only at-rest (A19) and bypasses the substitute presence factor. | Manifest install script (ops/02 §3): if `secrets.toml` has `profile="remote"`, REQUIRE `operator_authorizer_url` configured else FATAL exit 1 (fail-closed). Until OI-SM-2/3 land, gate must be present; any VPS-config merge in CI must also add OI-SM-2/3 or fail (mirror the `enable=false` gate). | high |
| F8 | **RESOLVED (TASK-0033)** | VPS operator-box authorizer protocol specified + built (OI-SM-2) | `SERVER-MODE §5.3` lines 172-177; OI-SM-2 line 321; FS-S21/S23 | Without token format/binding/TTL/replay-window/outage behavior, a VPS deploy cannot exist; a weak binding invites token-mint from a compromised box or replay. | Design + test: Ed25519-signed token {ts, vps_instance_id, nonce, expiry, replay_nonce}; bound to VPS mTLS cert + per-request server nonce; short TTL (5-15 min forcing re-mint that re-checks USB); replay defense (server nonce/jti); operator-box-unreachable ⇒ drain + deny (fail-closed). Negative tests for FS-S21/S23 must pass before Profile B is shippable. | high |
| F9 | **RESOLVED (TASK-0033)** | Wrapping-vs-gating separation enforced (per-swap gate re-resolve, FS-S23) | `SERVER-MODE §5.3` line 177; FS-S23 line 303; `research/15 §2` | A `remote_release` keyslot unwraps the DEK at boot; if the boot-unwrapped DEK is cached and the presence token later goes `Unproven`, egress could continue without a valid presence factor — violating fail-closed. | Phase B: at startup in VPS mode assert PresenceGate not `Unproven` before any swap (FS-S21); on EVERY `relay_swap` re-resolve the gate (never cache the result across swaps); `Unproven` at swap ⇒ deny `GateAbsent`. The vault DEK cache MUST NOT be the relay path's authority. | high |
| F10 | **HIGH** | CVE-2024-47609 fix is now encoded as explicit dependency floors + CI audit gate | `SERVER-MODE §6.2`; `Cargo.toml` pins workspace floors `tonic = "0.12.3"` and `hyper = "1.10.1"` with comments; `.github/workflows/ci.yml` runs `ci/gates/cargo-audit.sh` | Accept-loop DoS on the NEW network listener (NOT mitigated by "control is UDS-only") now fails CI if the lockfile drifts to a vulnerable tonic/hyper line. | DONE in TASK-0034 and hardened in the 2026-07-13 dependency repair. `cargo audit` has no vulnerability exceptions: the advisory-affected RustCrypto `rsa` path was replaced by the exact-pinned fixed-width constant-time signer stack, and `time` is fixed at the MSRV-compatible line. | high |
| F11 | **HIGH** | MSRV 1.89 now verified against the real lockfile (HF-18 / OQ-1) | `Cargo.toml` `rust-version="1.89"`; `.github/workflows/ci.yml` MSRV job; `Cargo.lock` pins transitive patches that otherwise drifted above 1.89 | Silent transitive MSRV drift is now caught by `cargo +1.89.0 check --workspace --locked`. The parent workspace deliberately does not use `--all-features` because the libSQL store features are mutually exclusive. | DONE in TASK-0034. Future dependency updates must keep the MSRV job green or raise the floor with an explicit design decision. | medium |
...
| F16 | RESOLVED | mTLS hardened-mode client-cert lifecycle / revocation-propagation specified and implemented (OI-SM-4) | TASK-0039 / PR #162; `SERVER-MODE §9` resolution note | Leaf issuance/TTL/auto-renewal and revocation-set propagation are no longer undefined for the remote-clients CA lifecycle. | Implemented: separate remote-clients CA; ≤7d leaves; `Certs.Renew` revokes superseded leaves; `Certs.Revoke` appends SHA-256 DER fingerprints in the verifier reload format from PR #158. Remaining root-of-trust verbs stay explicit `Unimplemented`: `Certs.CaRotate` and `Certs.TrustApply`. | high |
...
| FS-S21 VPS refuses to start w/o substitute factor | **RESOLVED** (F7/TASK-0033) | install-hook FATAL + assert_vps_factor_configured |
| FS-S22 on-box w/o USB keyslot refuses | **RESOLVED** (F22/TASK-0033) | assert_onbox_usb_keyslot_or_override + --allow-passphrase-only |
| FS-S23 no boot-unwrap egress w/o presence token | **RESOLVED** (F9/TASK-0033) | per-swap gate re-resolve + assert_gate_not_unproven_at_startup |
...
2. **F10 — DONE (TASK-0034)** — `tonic = "0.12.3"` + named patched `hyper = "1.10.1"` are encoded
   in `Cargo.toml`, and `ci/gates/cargo-audit.sh` runs in CI.
3. **F11 — DONE (TASK-0034)** — `cargo +1.89.0 check --workspace --locked` is a CI job; the
   lockfile pins transitive patches that drifted above Rust/Cargo 1.89. `--all-features` is
   intentionally invalid in this parent workspace because store features are mutually exclusive.
...
**Profile B (VPS) shippability gate — ALL RESOLVED as of TASK-0033:**

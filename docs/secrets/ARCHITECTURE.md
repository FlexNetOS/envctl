...
The secrets-only rows (tokio, tonic, prost, tonic-build, tower, hyper, hyper-util, http-body-util, reqwest, chacha20poly1305, argon2, hkdf, sha2, blake3, zeroize, subtle, rand, getrandom, rcgen, rustls, rustls-pki-types, x509-parser, webpki-roots, async-trait, tracing-subscriber, the chosen pure-Rust store) are additive and pulled ONLY by the new crates. PEM decoding uses the maintained `rustls-pki-types::pem::PemObject` API directly. CLI verbs fold under `envctl secret|vault|relay|ca|run`; the Yazelix flake pins `secretd` and its stack bootstrap owns its absolute runtime path, startup order, readiness, and USB unlock. The `SecGuard`/`GuardRefused`/resolve-once-`UnlockContext` idiom is the SAME as `envctl-engine`'s `guard.rs`; REQ-SEC-*/FS-S* sit beside REQ-SAFE-*/FS-1..8.

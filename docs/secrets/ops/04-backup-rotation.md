...
1. **Back up exactly one file: `$META_ROOT/var/lib/env-ctl/vault.db`** (dir `0700`,
...
`$META_ROOT/var/lib/env-ctl/vault.db` is the **sole required** target
(`ARCHITECTURE.md` lines 125–127; `schema.sql` line 15). Everything else under
`$META_ROOT/var/lib/env-ctl/` is either public (`ca/ca.pem`, `0644`) or
regenerable. The audit mirror at `$META_ROOT/var/lib/env-ctl/` (`0700`) is a
...
sqlite3 "$META_ROOT/var/lib/env-ctl/vault.db" "VACUUM INTO '$SNAP'"
chmod 0600 "$SNAP"
```
...
cp --reflink=auto "$META_ROOT/var/lib/env-ctl/vault.db" "$SNAP"
chmod 0600 "$SNAP"
```
...
install -m 0600 "$SNAP" "$META_ROOT/var/lib/env-ctl/vault.db"
...
rm -f "$META_ROOT/var/lib/env-ctl/vault.db"

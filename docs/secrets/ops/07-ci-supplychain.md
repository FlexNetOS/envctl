...
**Verified-against (refreshed 2026-07-12):** the workspace declares `rust-version = "1.89"`, `rust-toolchain.toml` keeps latest nightly as the developer default, and CI verifies the independent floor with `cargo +1.89.0 check --workspace --locked`; RustSec advisories remain enforced by `ci/gates/cargo-audit.sh`.
...
| MSRV `cargo +1.89.0 check` | HF-18 — a silent transitive MSRV bump past the declared floor | A latest-nightly developer build hides a 1.89 break; only the exact 1.89.0 toolchain surfaces it. |
...
## 2. MSRV gate (Rust 1.89)

### 2.1 Policy

- **Declared floor:** `rust-version = "1.89"` in `[workspace.package]` (workspace `Cargo.toml`).
- **Dev toolchain:** `rust-toolchain.toml` stays on latest `nightly`. The 1.89 floor is a *separate, exact, verified* CI gate and is not the developer default.
- **Verified gate:** `cargo +1.89.0 check --workspace --locked` is now a required CI job. The
  ...
  this gate, either pin the affected package to a 1.89-compatible patch or raise the shared floor with
...
```yaml
  msrv:
    ...
    steps:
      ...
      - uses: dtolnay/rust-toolchain@1.89.0          # pinned-version form, https://github.com/dtolnay/rust-toolchain
      ...
      - run: cargo +1.89.0 check --workspace --locked
```

Optional hardening (recommended): add `cargo-msrv` as an *advisory* (non-gating) job to auto-discover the true floor and warn if it drifts above 1.89, so the team sees a floor bump coming. https://github.com/foresterre/cargo-msrv
    ...
    steps:
      ...
      - uses: EmbarkStudios/cargo-deny-action@v2
        with:
          ...
          # rust-version: 1.89.0   # optionally evaluate advisories against the MSRV graph
```
...
jobs:
  lint:               # §5
  msrv:               # §2.2
  test:               # §3  (matrix: stable + 1.89.0 x feature rows)
...
- `dtolnay/rust-toolchain` — https://github.com/dtolnay/rust-toolchain (ref form `@stable` / `@1.89.0`)
...
| OQ-7 | **`cargo audit` vs `cargo deny check advisories`** overlap | LOW | RESOLVED for TASK-0034 and hardened in the 2026-07-13 dependency repair: `cargo audit` is the advisory CI gate and carries no vulnerability exceptions. The RustCrypto `rsa` dependency was replaced by the exact-pinned fixed-width constant-time signer stack, while `time` remains on its fixed MSRV-compatible line. Add `cargo-deny` later only for licenses/bans/sources if needed, not as a second advisory authority. |
...
| OQ-10 | **Where gates run pre-merge into envctl** (Phase 7) | MEDIUM | On merge into `envctl/crates/`, all gates here must re-run in the *parent* workspace (re-resolved single lockfile, `rustix` row hand-unioned to `["process","net"]`, HF-17). Confirm envctl's MSRV ≥ 1.89 or raise its floor with sign-off. |

...
* **Tier 2 — `nvidia-smi` / `nvcc` enrichment (best-effort, hard timeout).**
  ...
  enriches the GPU list and runtime telemetry; `nvcc --version` supplies the CUDA toolchain
  version. Both are optional enrichments, never the source of truth for driver presence.
...
nvidia-smi is absent/failing → telemetry stays empty, never a panic. CPU+mem come from `sysinfo`
(or `/proc/stat` + `/proc/meminfo` directly to stay lean). The sampler emits `Event::Telemetry`
every ~1s while the Dashboard is active, backing off to ~3-5s on other screens and pausing when the
window is unfocused — so it never needlessly spawns nvidia-smi.

...
- **Install never hijacks a name.** It refuses to frontdoor wrapper an artifact whose name
...
- **Refuse-overwrite-unmanaged.** A target is "ours" only if it's a frontdoor wrapper whose
...
- `verify` = `<bin> --version`;
- `remove` = excise **only our** frontdoor wrappers (readlink-into-store guard) then drop the clone;
...
envctl reset pastel --apply # removes frontdoor wrappers + drops the clone
```

...
When launched by the **`auto-provision`** runner (a fresh `Codex -p` process per cycle — the
...
- **Idempotent + declarative + rust-native.** Treat a non-Rust source/package file that appears as
  drift to fix, not accept (see AGENTS.md). Re-running a healthy component must be a no-op.

...
process** is a clean context — so an external shell loop spawns a fresh `Codex -p` each iteration.
...
2. spawns one fresh `Codex -p "<resume prompt>"` (clean context) in the worktree,

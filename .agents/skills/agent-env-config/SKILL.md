...
- **Codex** → `.Codex/` : skills under `.Codex/skills/<name>/SKILL.md`, plus `.Codex/settings*.json`. Do NOT hand-maintain `.Codex/homunculus/instincts/...` ECC files — they're superseded by these curated skills.
...
`github`, `context7`, `exa`, `memory`, `playwright`, `sequential-thinking`. This pack is the standard tool surface for envctl work; provision it through kasetto so Codex (`.mcp.json`/settings) and Codex (`config.toml` `[mcp_servers.*]`) stay in lockstep rather than drifting.

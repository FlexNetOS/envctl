...
target/port repo's `.Codex/` (see §Eject). Built on the FlexNetOS autonomous-operation pattern:
...
repo's `.Codex/` and scaffolds `.handoff/loop/`. See `references/eject.md`. Invoke as `/rust-port`
once ejected.

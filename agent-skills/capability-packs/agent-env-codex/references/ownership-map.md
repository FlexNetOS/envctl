...
| `agent-skills/capability-packs/agent-env-codex/` | Durable source of this one skill | Git-tracked owner; must match every projection/materialization. |
...
| Text skill surfaces under `agent-skills/`, `.codex/skills/`, `.claude/skills/`, and `home/*/skills/` | Executable Markdown recipes, scripts, code launchers, and command-bearing config loaded by agents | Follow `bunx-and-github-ssh.md`; no executable `npm`/`npx`, no global package install, and validate every supported text surface with `check-bun-command-policy.py`. |
...
| `/home/flexnetos/.codex/skills/agent-env-codex/` | Active skill materialization | Must match `agent-skills/capability-packs/agent-env-codex/`; runtime activation is not the durable owner. |

...
#
# Fail-closed: the committed agent-env.yaml must match agent-env.lock. `envctl agent lock --check`
# is read-only, zero-network (no fetch), and exits 1 on config<->lock drift. Sibling to
# ci/gates/{no-c,shape,enable,p7}.sh.
set -euo pipefail
...
if "$BIN" agent lock --check; then
  echo "AGENT-ENV GATE PASS"
else
  echo "AGENT-ENV GATE FAIL — agent-env.yaml drifted from agent-env.lock" >&2
  echo "  fix: 'envctl agent lock' to rewrite the lock, then commit agent-env.lock" >&2
  exit 1
fi

...
`./.handoff/ledger.db` local cache + committed `ledger.events.jsonl`; run hf in the member repo, then `hf export`/`hf sync` as needed; see the `handoff-sync` skill):
...
- **Per-repo vector (A2 only)** — when the in-flight cycle ran A2 (>1 target repo), capture the
  ...
  claims, verify-on-resume cmd}`. Confirm the set still exists only after deriving the managed
  set slug from the path shape: `scripts/reap-worktrees.sh --managed-worktree-slug <worktree-dir> <repo>`
  must succeed before running `meta git worktree status <slug>`. The repo/project name (`envctl`) is
  not a slug for the main checkout; single-repo/main-checkout resumes omit this status check.

...
description: >-
  ...
  nushell-primary shell doctrine (bash native inside the configured nu runtime — no shell
  wrapper; Nu/Rust wrappers only), wire the six substrates
...
(ccbrain / rtk / weave wired; retired bash-to-nu / launcher wrappers ABSENT), the nushell owner
surfaces (`~yazelix/nushell/{config,scripts}` projection) + the native-bash-in-nu probe, yzx
doctor warning sweep, stale-shadow scan, and

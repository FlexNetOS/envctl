...
for s in planning-engineer plan-loop plan-cartography plan-trend-research plan-governance-config plan-filesystem-layout plan-test-strategy plan-synthesis code-research-verify; do
  [ -d "$TARGET/.claude/skills/$s" ] || fail "OWN skill not copied: $s"
done
for a in plan-cartographer plan-trend-researcher plan-governance-config-auditor plan-filesystem-layout-auditor plan-analyst plan-test-strategist plan-verifier plan-architect; do

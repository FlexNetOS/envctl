...
7. **SSH Git transport.** Run `rtk meta git setup-ssh`, require FlexNetOS remotes to use the organization SSH form, and verify fetch/push identity before mutation. SSH authenticates Git transport; organization administration still uses `gh`, REST, GraphQL, or the GitHub UI.

...
def precedence-rows [] {
    [
        ...
        (precedence-row "codex_global_config" "codex" "codex" "codex" "codex_config" "/home/flexnetos/.codex/config.toml" "env_files.csv" "codex_config" 20 "explicit_config" "redacted_reference_only" "Global Codex config is an explicit config layer after runtime env and credential-store policy." "Project config conflicts must be reported rather than silently merged." "review" "T028,T034" "Codex global config precedence.")
        ...
        (precedence-row "yazelix_envctl_template" "yazelix" "envctl" "yazelix" "nix-yazelix manifest" "/home/flexnetos/FlexNetOS/src/envctl/manifest/nix-yazelix.toml" "env_files.csv" "envctl_manifest_nix_yazelix" 30 "repo_manifest" "public_config_ref" "Envctl Yazelix manifest supplies repo defaults and install intent." "Generated overlays require explicit diff before apply." "review" "T028,T034" "Yazelix manifest layer.")
        (precedence-row "yazelix_nushell_path_template" "yazelix" "envctl" "shell" "PATH" "/home/flexnetos/FlexNetOS/src/envctl/home/.config/nushell/meta-usr-path.nu" "nushell_env_config_parsed.csv" "env_assignment:PATH" 30 "repo_template" "public_config_ref" "Envctl Nushell template prepends META_ROOT bins and de-duplicates PATH only after existing env is present." "Path order merges emit inspectable conflict rows." "review" "T031,T034" "Nushell/Yazelix PATH layer.")

...
def precedence-rows [meta: string] {
    [
        ...
        (precedence-row "codex_global_config" "codex" "codex" "codex" "codex_config" "/home/flexnetos/meta/var/lib/codex/config.toml" "env_files.csv" "codex_config" 20 "explicit_config" "redacted_reference_only" "Global Codex config is an explicit config layer after runtime env and credential-store policy." "Project config conflicts must be reported rather than silently merged." "review" "T028,T034" "Codex global config precedence.")
        (precedence-row "codex_project_config" "codex" "envctl_codex" "codex" "project_codex_config" $"($meta)/src/envctl/.codex/config.toml" "env_files.csv" "envctl_codex_project_config" 22 "project_config" "redacted_reference_only" "Project Codex config scopes repo-local behavior and must not mutate global auth state." "Global/project setting collisions require inspectable rows." "review" "T028,T034" "Codex project config precedence.")
        (precedence-row "codex_state_table" "codex" "envctl" "codex" "state_locations" $"($meta)/var/lib/envctl/tables/codex_state_locations.csv" "env_files.csv" "envctl_table_codex_state" 40 "table_defaults" "path_only_or_redacted_reference" "Codex state locations are envctl table references, not raw state content." "Generated Codex fragments must not copy state database contents." "required" "T028,T034" "Codex state table precedence.")
        ...
        (precedence-row "yazelix_envctl_template" "yazelix" "envctl" "yazelix" "nix-yazelix manifest" $"($meta)/src/envctl/manifest/nix-yazelix.toml" "env_files.csv" "envctl_manifest_nix_yazelix" 30 "repo_manifest" "public_config_ref" "Envctl Yazelix manifest records validation intent; only merged Yazelix origin/main owns installation." "Generated projections require explicit diff before apply." "review" "T028,T034" "Yazelix manifest layer.")
        (precedence-row "yazelix_nushell_path_template" "yazelix" "envctl" "shell" "PATH" $"($meta)/src/envctl/home/.config/nushell/profile-path.nu" "nushell_env_config_parsed.csv" "env_assignment:PATH" 30 "repo_template" "public_config_ref" "Envctl Nushell validation input replaces inherited PATH with the current profile toolbin/bin pair." "Any other command owner is a fail-closed conflict." "required" "T031,T034" "Strict Nushell/Yazelix profile PATH layer.")

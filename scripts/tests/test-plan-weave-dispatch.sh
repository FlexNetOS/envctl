...
out="$(bash "$root/scripts/plan-weave-dispatch.sh" --dry-run --target rusty-idd --root "$tmp" --run-id test-run --state-dir "$tmp/dispatch")"
...
if PLAN_OPUS_CMD="$tmp/no-such-claude" bash "$root/scripts/plan-weave-dispatch.sh" --target rusty-idd --root "$tmp" --run-id fail-run --state-dir "$tmp/real" >/tmp/plan-weave-dispatch.out 2>/tmp/plan-weave-dispatch.err; then

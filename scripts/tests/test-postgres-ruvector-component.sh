...
reset_fixture() {
  ...
  mkdir -p "$data/pg_tblspc" "$meta/var/lib/ruvector" \
    "$real_home/.local/state/nix/profiles" \
...
EOF
  ...
  ln -s "$profile_generation" "$real_home/.local/state/nix/profiles/profile-1-link"
  ln -s profile-1-link "$real_home/.local/state/nix/profiles/profile"
  ln -s "$real_home/.local/state/nix/profiles/profile" "$real_home/.nix-profile"

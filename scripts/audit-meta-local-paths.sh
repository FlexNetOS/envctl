...
like .ideavimrc, portable app-config dirs like .gphoto/.vscode-shared/.archon/.n8n-mcp, or a managed dotfile present under --envctl-home-source).
...
is_portable_app_config_dir_dot() {
  case "$1" in
    .gphoto|.vscode-shared|.repomix|.archon|.n8n-mcp) return 0 ;;
    *) return 1 ;;
  esac
}
...
canonical_target_for_dot() {
  ...
  case "$dot" in
    .cargo|.rustup|.bun|.npm|.wasmer|.dotnet|.pgrx|.venvs|.go|.gradle|.nix-*)
      ...
      ;;
    .agents|.ampcode|.archon|.claude|.claude.json|.codex|.codeium|.copilot|.cursor|.gemini|.goose_recipes|.gphoto|.vscode-shared|.repomix|.junie|.kimi|.kimi-code|.n8n-mcp|.ollama|.roo|.vscode|.windsurf|.mozilla|.thunderbird|.ideavimrc)

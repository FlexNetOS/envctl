...
def main [--json] {
    ...
    let rows = [
        ...
        (tool-row wild "Wild linker" linker (command-stdout "wild --version") $"($meta)/etc/rust/wild/cargo-wild-x86_64-unknown-linux-gnu.toml" toml T025 workspace_config $"pinned:($wild_release.path)#($wild_release.version_or_rev)" source_of_truth false $wild_binary.path (sha-file $"($meta)/etc/rust/wild/cargo-wild-x86_64-unknown-linux-gnu.toml") parsed public "Wild is opt-in through a workspace Cargo config profile.")
        ...
        (rust-toolchain-row envctl_rust_toolchain "envctl Rust toolchain" $"($meta)/src/envctl/rust-toolchain.toml" T032 "envctl pins the development toolchain.")
        (rust-toolchain-row runner_rust_toolchain "runner Rust toolchain" $"($meta)/flexnetos_runner/rust-toolchain.toml" T032 "runner currently uses a floating stable toolchain.")
        (rust-toolchain-row meta_rust_toolchain "meta Rust toolchain" $"($meta)/rust-toolchain.toml" T032 "meta currently uses a floating stable toolchain.")
        (tool-row meta_tool_versions "meta .tool-versions" tool_versions "rust stable" $"($meta)/.tool-versions" text T032 repo_config "floating:stable" source_of_truth false "" (sha-file $"($meta)/.tool-versions") parsed public "meta asdf-style tool version file declares rust stable.")
        (rust-toolchain-row kache_rust_toolchain "kache Rust toolchain" $"($meta)/src/upstream/kunobi-ninja/kache/rust-toolchain.toml" T032 "upstream Kache source pins Rust 1.95.")
        (tool-row kache_mise "Kache mise tools" tool_manager "see_mise_toml" $"($meta)/src/upstream/kunobi-ninja/kache/mise.toml" toml T032 upstream_config "mixed:pinned-plus-rust-toolchain" source_of_truth false "" (sha-file $"($meta)/src/upstream/kunobi-ninja/kache/mise.toml") parsed public "mise.toml pins CI tools and defers Rust to rust-toolchain.toml.")
        (tool-row kache_flake "Kache flake" nix_flake "nixpkgs-unstable" $"($meta)/src/upstream/kunobi-ninja/kache/flake.nix" nix T032 upstream_config "flake.lock present" source_of_truth false "" (sha-file $"($meta)/src/upstream/kunobi-ninja/kache/flake.nix") parsed public "Kache upstream flake follows nixpkgs and rust-overlay inputs.")
        (tool-row wild_flake "Wild flake" nix_flake "nixos-unstable" $"($meta)/src/upstream/wild-linker/wild/flake.nix" nix T032 upstream_config "flake.lock present" source_of_truth false "" (sha-file $"($meta)/src/upstream/wild-linker/wild/flake.nix") parsed public "Wild upstream flake uses nixos-unstable and crane.")
        (tool-row gitkb_config "GitKB config" memory_tool "configured" $"($meta)/src/envctl/.kb/config.toml" toml T032 repo_config "auth:optional" source_of_truth false "" (sha-file $"($meta)/src/envctl/.kb/config.toml") parsed public "GitKB repo config is present; no GitKB binary was found on ambient PATH.")
        (tool-row flexnetos_runner_workspace "FlexNetOS runner workspace" runner "0.1.0" $"($meta)/flexnetos_runner/Cargo.toml" toml T032 repo_config "workspace.package.version=0.1.0" source_of_truth false "" (sha-file $"($meta)/flexnetos_runner/Cargo.toml") parsed public "Runner workspace manifest provides the current runner package version.")
    ]

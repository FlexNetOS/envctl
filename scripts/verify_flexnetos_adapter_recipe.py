...
try:
    ...
    _COMMON_CANDIDATES = [
        ...
        / "scripts"
        / "_common.py",
        Path("/home/flexnetos/meta/src/envctl")
        / "envctl-db-nu-plugin-migration-automation-package"
        / "execution-framework"
        / "scripts"
        / "_common.py",
    ...
    for _path in _COMMON_CANDIDATES:
        if not _path.exists():
            continue
        _spec = importlib.util.spec_from_file_location("_common", _path)
        if _spec and _spec.loader:
            _module = importlib.util.module_from_spec(_spec)
            _spec.loader.exec_module(_module)
            append_proof = _module.append_proof
            make_proof = _module.make_proof
            now = _module.now
            package_root = _module.package_root

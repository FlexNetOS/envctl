...
pub struct DashboardPlan {
    ...
    pub kdl: String,
    /// Where `deploy` would write (`$META_ROOT/.config/yazelix/.../<name>.kdl` unless XDG_CONFIG_HOME is explicitly set).
    pub target: PathBuf,
}
...
/// Resolve the deploy target: `$META_ROOT/.config/yazelix/configs/zellij/layouts/<name>.kdl` unless XDG_CONFIG_HOME is explicitly set.
pub fn layout_target(name: &str) -> PathBuf {
    let base = std::env::var("XDG_CONFIG_HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|_| MetaLayout::from_env_or_default().xdg_config_home());

...
impl Catalog {
    ...
    pub fn validate(&self, project_root: &Path) -> Result<()> {
        ...
        let core_pack = self
            .packs
            .get(CORE_PACK)
            .ok_or_else(|| err("mandatory core pack is missing while already validated"))?;
        if !core_pack.skills.iter().any(|skill| skill == "skill-catalog") {
            return Err(err(
        ...
        self.skills
            ...
            .map(|name| {
                let skill = self
                    .skills
                    .get(name)
                    .ok_or_else(|| err(format!("active skill `{name}` is missing from catalog metadata")))?;
                Ok(CatalogSkill {
                    name: name.clone(),
                    owner: source_for(name, skill)?,
                    packs: self
                        .packs
                        .iter()
    ...
    pub fn render_active_projection(&self) -> Result<String> {
        ...
        for name in skills {
            let skill = self
                .skills
                .get(&name)
                .ok_or_else(|| err(format!("active catalog skill `{name}` is missing from metadata")))?;
            ...
            if source.starts_with("https://") {
                let source_ref = skill
                    .source_ref
                    .as_deref()
                    .ok_or_else(|| err(format!("active catalog skill `{name}` missing source ref for remote source `{source}`")))?;
                let sub_dir = skill
                    .sub_dir
                    .as_deref()
                    .ok_or_else(|| err(format!("active catalog skill `{name}` missing sub-dir for remote source `{source}`")))?;
                out.push_str(&format!(
                    "    ref: {}\n    sub-dir: {}\n    skills:\n      - {name}\n",
                    source_ref, sub_dir,
                ));
...
mod tests {
    ...
    fn catalog_validates_owner_sources_and_resolves_compact_metadata() {
        let (_root, mut catalog) = catalog_with_sources();
    ...
    #[test]
    fn catalog_rejects_render_active_projection_with_missing_remote_fields() {
        let (root, mut catalog) = catalog_with_sources();
        catalog.skills.insert(
            "remote".into(),
            Skill {
                source: Some("https://github.com/FlexNetOS/meta".into()),
                source_ref: None,
                sub_dir: None,
                aliases: Vec::new(),
                intents: Vec::new(),

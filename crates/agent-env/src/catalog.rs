...
impl Catalog {
    ...
    pub fn validate(&self, project_root: &Path) -> Result<()> {
        ...
        for (name, skill) in &self.skills {
            ...
            let root = skill_root(project_root, name, skill)?;
            if !root.join("SKILL.md").is_file() {
                return Err(err(format!(
                    "catalog skill `{name}` has no canonical SKILL.md at {}",
                    root.display()
                )));
            }
    ...
    pub fn render_active_projection(&self) -> Result<String> {
        ...
        for name in skills {
            let source = source_for(&name, self.skills.get(&name).expect("active key exists"))?;
...
fn skill_root(project_root: &Path, name: &str, skill: &Skill) -> Result<PathBuf> {
    let source = source_for(name, skill)?;
    Ok(project_root.join(source.trim_start_matches("./")))
}
...
fn source_for(name: &str, skill: &Skill) -> Result<String> {
    let source = skill.source.clone().unwrap_or_else(|| {
        if name == "skill-catalog" {
            ...
            format!("./agent-skills/capability-packs/{name}")
        }
    });
    ...
    let trusted = normalized == "agent-skills/skill-catalog"
        || normalized.starts_with("agent-skills/capability-packs/")
...
mod tests {
    ...
    fn catalog_with_sources() -> (TestRoot, Catalog) {
        ...
        for name in ["a", "b"] {
            let dir = root.path().join("agent-skills/capability-packs").join(name);
    ...
    fn catalog_rejects_missing_owner_and_unsafe_keyword() {
        let (root, mut catalog) = catalog_with_sources();
        fs::remove_file(root.path().join("agent-skills/capability-packs/a/SKILL.md")).unwrap();
        assert!(catalog
            .validate(root.path())
            .unwrap_err()
            .to_string()
        ...
        fs::write(
            root.path().join("agent-skills/capability-packs/a/SKILL.md"),
            "---\nname: test\n---\n",
        )
        .unwrap();
